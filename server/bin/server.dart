...
import 'package:server/src/generated/user.pbgrpc.dart';
...
class GrpcChat extends GrpcChatServiceBase {
  ...
  Future<CreateMessageResponse> createMessage(
    ...
    var messageFromBase = MessageFromBase(
        chatIdMain: mes['chat_id'],
        senderMainId: mes['sender_id'],
        content: mes['content'],
        date: mes['created_date'],
        mainIdMessage: mes['message_id']);

    var receiverId = usersService.getUserIdByChat(
        senderId: request.senderMainId, chatId: request.chatIdMain);

    var hashConnect = usersService.getHashCodeById(id: receiverId);
    hashConnect ?? _controller[hashConnect]?.sink.add(messageFromBase);
    return message;
  }
  ...
  @override
  Stream<MessageFromBase> connecting(
